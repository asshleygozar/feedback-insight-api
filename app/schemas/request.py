...
class FeedbackRequest(BaseModel):
    ...
    def validate_open_ended_answer(cls, value):
        ...
        if not stripped_value:
            raise PydanticCustomError("open_ended_answer_empty", "Open-ended answer cannot be empty")
        if len(stripped_value) > 700:
            raise PydanticCustomError("open_ended_answer_too_long", "Open-ended answer cannot exceed 700 characters")
...
class BatchFeedbackRequest(BaseModel):
    ...
    def validate_feedbacks(cls, value):
        if len(value) > 50:
            raise PydanticCustomError("feedbacks_too_many", "Maximum of 50 feedbacks allowed in a single batch request")
        return value