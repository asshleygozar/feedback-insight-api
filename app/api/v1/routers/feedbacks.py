...
async def analyze(request: Request, data: FeedbackRequest, ai_service: AIService = Depends()) -> FeedbackResponse:
    ...
    if response is None:
        logger.error(f"Failed to analyze feedback for request {request.url.path} - {request.method}", exc_info=True)
...
async def batch_analyze(request: Request, data: BatchFeedbackRequest, ai_service: AIService = Depends()) -> FeedbackResponse:
    ...
    if responses is None:
        logger.error(f"Failed to analyze feedbacks for request {request.url.path} - {request.method}", exc_info=True)
